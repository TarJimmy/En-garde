...
public class ControlerJeu extends Controler {
	...
	protected void InsertJoueursBD() {
	...
	public boolean clickCase(int x, int nbCartesAUtiliser) {
		...
		if (jeu.jouer(coupAJouer, false)) {
			...
			jeu.listeEscrimeurAjouer.add(jeu.getIndiceCurrentEscrimeur());
			return finirAction();
		} else {
			jeu.modifieVue(Action.ACTUALISER_PLATEAU);
	...
	public boolean animation(String commande, Animation anim) {
		switch (commande) {
			...
			case "Lancer":
				...
				return true;
			default:
				System.err.println("Animation pas traité : " + commande);
				break;
			}
		return false;
	}
	...
	public boolean commande(String c) {
		switch (c) {
			...
			case "annuleCoup":
				...
				if (jeu.getCurrentEscrimeur().isIA) {
					jeu.getHistorique().annulerCoup();
				}
				return false;
			case "refaireCoup":
			...
			case "montrerAide":
				...
				return true;
			default:
				System.err.println("Commande pas traitee : " + c);
				return false;
		}
	}
	...
	public boolean finirAction() {
		...
		if(cases.size() == 1 && cases.contains(-1)) {
			...
			if (jeu.getCurrentEscrimeur().getType() == TypeEscrimeur.HUMAIN) {
				jeu.modifieVue(Action.ACTUALISER_PLATEAU);
				jeu.modifieVue(jeu.getIndiceCurrentEscrimeur() == Escrimeur.GAUCHER ? Action.ACTUALISER_ESCRIMEUR_GAUCHER : Action.ACTUALISER_ESCRIMEUR_DROITIER);
			} else {
				jeu.IADoitJouer();
			}
		}
	...
	public void nouveauMatch() {
		closeAll();
		jeu.indicePremierJoueurPartie = (jeu.indicePremierJoueurPartie + 1) % 2;
		nouvellePartie();
