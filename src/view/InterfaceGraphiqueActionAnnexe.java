...
public class InterfaceGraphiqueActionAnnexe extends WindowAdapter implements Runnable, Observateur {
	...
	public static void demarrer(CollecteurEvenements control) {
		System.out.println("fenetreActive : " + fenetreActive);
		if (!fenetreActive) {
			fenetreActive = true;
		...
		try {
			...
			case "Titre":
				banner = new ImageIcon(
						new ImageIcon(ImageIO.read(Configuration.charge("cadre.png", Configuration.MENU))).getImage()
								.getScaledInstance(250, 75, Image.SCALE_SMOOTH));
			...
			default:
				banner = new ImageIcon(
						new ImageIcon(ImageIO.read(Configuration.charge("cadre4.png", Configuration.MENU))).getImage()
	...
	public void run() {
		try {
			fenetreActionAnnexe = new JFrame("EN GARDE ! - Action Annexes");
			fenetreActionAnnexe.setIconImage(Configuration.imgIcone);
			fenetreActionAnnexe.addWindowListener(this);
			...
			contentPane = new JLabel(
					new ImageIcon(ImageIO.read(Configuration.charge("ActionAnnexe.png", Configuration.MENU))));
			contentPane.setBorder(new EmptyBorder(5, 5, 5, 5));
			fenetreActionAnnexe.setContentPane(contentPane);
			try {
				fenetreActionAnnexe.setCursor(Toolkit.getDefaultToolkit().createCustomCursor(
						ImageIO.read(Configuration.charge("curseur.png", Configuration.AUTRES)), new Point(0, 0),
						"Mon curseur"));
			...
			JButton activeAide = new ButtonCustom("Montrer meilleur coup", "cadre2", new Dimension(200, 50), font);
			activeAide.addActionListener(new AdaptateurCommande(controle, "montrerAide"));
