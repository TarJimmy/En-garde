...
public class InterfaceGraphiqueJeu implements Runnable, Observateur {
	...
	public static InterfaceGraphiqueJeu graphicJeu;
	...
	public static void demarrer(CollecteurEvenements control, Jeu jeu) {
		if (graphicJeu == null) {
			graphicJeu = new InterfaceGraphiqueJeu(control, jeu);
			SwingUtilities.invokeLater(graphicJeu);
		} else {
			jeuPresent = true;
			graphicJeu.controle = control;
			graphicJeu.jeu = jeu;
			graphicJeu.frame.setVisible(jeuPresent);
			graphicJeu.jeu.ajouteObservateur(graphicJeu);
			graphicJeu.controle.commande("PageInitialiser");
		}
	}
	
	public static void demarrer(ControlerJeu controlerJeu) {
		if (graphicJeu == null) {
			graphicJeu = new InterfaceGraphiqueJeu(controlerJeu, controlerJeu.getJeu());
		} else {
			jeuPresent = true;
			graphicJeu.frame.setVisible(jeuPresent);
		}
	...
	public void miseAJour() {
		...
		switch (action) {
			...
			case ACTUALISER_PLATEAU: // Actualise les cases accessibles du plateau
				...
				vueEscrimeurs[indiceCurrent].setBtnPasserTourVisibility(jeu.getEscrimeurs()[indiceCurrent].isIA && casesJouables.contains(-1));
			...
			case ACTUALISER_ESCRIMEUR_DROITIER :
				vueEscrimeurs[Escrimeur.DROITIER].actualise(jeu.popShowCarte(Escrimeur.DROITIER), (!jeu.isIA(Escrimeur.DROITIER) && jeu.getIsTourGaucher() == false) || jeu.getShowAllCartes(), jeu.casesJouables().contains(-1));
			...
			case ACTUALISER_ESCRIMEUR_GAUCHER:
				vueEscrimeurs[Escrimeur.GAUCHER].actualise(jeu.popShowCarte(Escrimeur.GAUCHER), (!jeu.isIA(Escrimeur.GAUCHER) && jeu.getIsTourGaucher()) || jeu.getShowAllCartes(), jeu.casesJouables().contains(-1));
				controle.commande("ActionTerminer");
				break;
	...
	public void run() {
		System.out.println("Lance fenetre feu");
