...
public class Jeu extends Observable {
	...
	public LinkedList<Integer> listeEscrimeurAjouer;
	...
	private int idJeu;
	public int indicePremierJoueurPartie;
	protected Jeu() {}
	...
	public Jeu(Boolean modeSimple, Plateau plateau, DeckPioche deckPioche, DeckDefausse deckDefausse, int nbManchesPourVictoire, int indiceCurrentEscrimeur, Escrimeur gaucher, Escrimeur droitier, int[] positionsDeparts, boolean animationAutoriser, int indicePremierJoueurPartie) {
		...
		this.indicePremierJoueurManche = (indicePremierJoueurPartie + 1) % 2;
	}
	...
	protected void init(Boolean modeSimple, Plateau plateau, DeckPioche deckPioche, DeckDefausse deckDefausse, int nbManchesPourVictoire, Escrimeur gaucher, Escrimeur droitier, int[] positionsDeparts, boolean animationAutoriser) {
		...
		this.listeEscrimeurAjouer = new LinkedList<>();
	...
	public int changerTour() {
		...
		if (escrimeurs[indiceCurrentEscrimeur].getType() != TypeEscrimeur.HUMAIN) {
			IADoitJouer();
		} else {
	...
	public void afficherEtatJeu() {
		System.out.println("au tour du joueur "+ getIndiceCurrentEscrimeur());
		...
		if (showGraphique) {
			switch (action) {
				...
				case ACTUALISER_DECK:
					...
					System.out.println(Arrays.toString(carteShowDeck.toArray()));
	...
	public void nouvellePartie() {
		...
		indicePremierJoueurManche = indicePremierJoueurPartie;
		indiceCurrentEscrimeur = indicePremierJoueurManche;
		nouvelleManche();
	}
	...
	public void clearAnimation() {
		if (showGraphique) {
			...
			this.listeIndicesPiocheRecemment[Escrimeur.GAUCHER].clear();
			this.listeIndicesPiocheRecemment[Escrimeur.DROITIER].clear();
	...
	public IA getIACurrent() {
		return getIA(escrimeurs[indiceCurrentEscrimeur].getType());
	}
	
	public IA getIA(TypeEscrimeur type) {
		switch (type) {
		case IA_FACILE:
			return new IA_Facile(this);

		case IA_MOYENNE:
			return new IA_Moyenne(this);
			
		case IA_DIFFICILE:
			return new IA_Difficile(this);
		default:
			System.out.println("C'est un humain");
			return null;
		}
	}
